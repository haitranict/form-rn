 ...
 */
export function useFormField(fieldConfig: FieldConfig) {
  ...
  const isRequiredByLogic = isFieldRequired(
    fieldConfig.conditional,
    typeof fieldConfig.validation?.required === 'boolean'
      ? fieldConfig.validation.required
      : (fieldConfig.validation?.required as { value: boolean })?.value ?? false,
    values
  );
