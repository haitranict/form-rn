...
export function useForm(config: FormConfig) {
  ...
  const [touched, setTouched] = useState<FormTouchedFields>({});
  ...
  const setFieldValue = useCallback(
    (fieldId: string, value: unknown) => {
      ...
      setErrors((prevErrors) => {
        ...
        const error = validateField(fieldCfg, value, { ...values, [fieldId]: value });
  ...
  const setFieldError = useCallback(
    (fieldId: string, error: import('../types/validation.types').FieldError | null) => {
      setErrors((prev) => {
        const next = { ...prev };
        if (error) {
          next[fieldId] = error;
  ...
  const goToNextPage = useCallback(() => {
    ...
    const touchedUpdates: FormTouchedFields = {};
    currentFields.forEach((f) => { touchedUpdates[f.id] = true; });
  ...
  const handleSubmit = useCallback(async () => {
    // Touch all fields
    const allTouched: FormTouchedFields = {};
  ...
  return {
    ...
    currentPage,
    totalPages,
    fields: allFields as FieldConfig[],
    pageFields,
    // setters
    setFieldValue,
    setFieldError,
