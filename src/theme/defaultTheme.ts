export interface FormTheme {
  // Colors
  colors: {
    primary: string;
    primaryLight: string;
    danger: string;
    dangerLight: string;
    warning: string;
    success: string;
    text: string;
    textSecondary: string;
    textDisabled: string;
    placeholder: string;
    border: string;
    borderFocused: string;
    background: string;
    inputBackground: string;
    labelBackground: string;
    checkmark: string;
    white: string;
  };
  // Typography
  typography: {
    fontSizeLabel: number;
    fontSizeInput: number;
    fontSizeHelper: number;
    fontSizeError: number;
    fontSizeSectionTitle: number;
    fontWeightLabel: '400' | '500' | '600' | '700';
    fontWeightSectionTitle: '400' | '500' | '600' | '700';
  };
  // Spacing
  spacing: {
    fieldGap: number;        // gap between fields
    fieldPaddingH: number;   // horizontal padding of input
    fieldPaddingV: number;   // vertical padding of input
    labelGap: number;        // gap between label and input
    helperGap: number;       // gap between input and helper/error
    sectionGap: number;      // gap above section headers
    pagePaddingH: number;    // form horizontal padding
    pagePaddingV: number;    // form vertical padding
  };
  // Shape
  shape: {
    borderRadius: number;
    borderWidth: number;
    borderWidthFocused: number;
  };
  // Input heights
  sizes: {
    inputHeight: number;
    checkboxSize: number;
    radioSize: number;
    switchWidth: number;
    switchHeight: number;
    starSize: number;
  };
}

export const defaultTheme: FormTheme = {
  colors: {
    primary: '#4285F4',
    primaryLight: '#D2E3FC',
    danger: '#EA4335',
    dangerLight: '#FCE8E6',
    warning: '#FBBC04',
    success: '#34A853',
    text: '#202124',
    textSecondary: '#5F6368',
    textDisabled: '#9AA0A6',
    placeholder: '#BDC1C6',
    border: '#DADCE0',
    borderFocused: '#4285F4',
    background: '#FFFFFF',
    inputBackground: '#FFFFFF',
    labelBackground: '#F8F9FA',
    checkmark: '#FFFFFF',
    white: '#FFFFFF',
  },
  typography: {
    fontSizeLabel: 14,
    fontSizeInput: 16,
    fontSizeHelper: 12,
    fontSizeError: 12,
    fontSizeSectionTitle: 20,
    fontWeightLabel: '500',
    fontWeightSectionTitle: '600',
  },
  spacing: {
    fieldGap: 16,
    fieldPaddingH: 12,
    fieldPaddingV: 10,
    labelGap: 6,
    helperGap: 4,
    sectionGap: 24,
    pagePaddingH: 16,
    pagePaddingV: 20,
  },
  shape: {
    borderRadius: 8,
    borderWidth: 1,
    borderWidthFocused: 2,
  },
  sizes: {
    inputHeight: 48,
    checkboxSize: 20,
    radioSize: 20,
    switchWidth: 44,
    switchHeight: 24,
    starSize: 32,
  },
};
