 ...
 */
export function evaluateConditional(conditional, allValues) {
  const results = conditional.rules.map(rule => evaluateRule(rule, allValues));
  return conditional.match === 'all' ? results.every(Boolean) : results.some(Boolean);
}

/**
 * Returns whether a field should be visible given form values.
 */
export function isFieldVisible(conditional, allValues) {
 ...
 */
export function isFieldRequired(conditional, baseRequired, allValues) {
 ...
 */
export function isFieldDisabled(conditional, baseDisabled, allValues) {
