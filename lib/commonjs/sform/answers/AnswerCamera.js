...
function AnswerCamera({
  ...
  const handleCameraCapture = () => {
    if (isMaxReached) {
      _reactNative.Alert.alert('Giới hạn', `Đã đạt giới hạn tối đa ${max} ảnh. Vui lòng xóa ảnh cũ trước khi chụp ảnh mới.`);
      return;
    }
    if (onCameraCapture) {
  ...
  const handleClear = () => {
    ...
    _reactNative.Alert.alert('Xoá ảnh', 'Vui lòng sử dụng nút xoá từng ảnh');
  };
  return /*#__PURE__*/_react.default.createElement(_reactNative.View, {
    style: styles.container
  }, myImages.length > 0 && /*#__PURE__*/_react.default.createElement(_reactNative.ScrollView, {
    horizontal: true,
    ...
    return /*#__PURE__*/_react.default.createElement(_reactNative.View, {
      ...
      onPress: () => {
        ...
        if (images.length > 0) {
          ...
          _reactNative.Alert.alert('Xoá ảnh', 'Tính năng xoá chưa được hỗ trợ cho camera-only type. Vui lòng sử dụng Image type.');
        } else {
          // Old pattern
    ...
    style: [styles.cameraText, isMaxReached && styles.cameraTextDisabled]
  }, "Ch\u1EE5p \u1EA3nh")));
}
