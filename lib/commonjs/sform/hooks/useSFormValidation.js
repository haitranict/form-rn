...
exports.useSFormValidation = useSFormValidation;
...
function useSFormValidation({
  ...
  const validate = (0, _react.useCallback)(() => {
            ...
            case 16:
              {
                ...
                if (question.min && Number(question.min) > 0) {
                  ...
                  if (total < Number(question.min)) {
                    onError(`Câu hỏi: ${question.questionName}. Yêu cầu số lượng audio tối thiểu là: ${question.min}`);
                    isCheck = false;
                    return {
                      isValid: false,
                      payload: null
    ...
    const payload = {
      ...
      formDate: parseInt((0, _moment.default)().format('YYYYMMDD'), 10),
